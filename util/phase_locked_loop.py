...
c_phase_locked_loop = ctypes.CDLL(
    "/Users/benjaminpattison/Documents/Projects/satNav/SDR_satellite_tracking/build/phase_locked_loop.so"
)
...
def phase_locked_loop(
    ...
    frequency_bandwidth,
    output_frequency_harmonic=None,
):
    ...
    if output_frequency_harmonic:
        ...
        c_phase_locked_loop.PhaseLockedLoopWithHarmonic(
            ...
            real_output,
            imaginary_output,
            real_output_harmonic,
            imaginary_output_harmonic,
            frequencies,
            ctypes.c_float(sampling_rate),
            ctypes.c_float(initial_frequency_estimate),
            ctypes.c_float(frequency_bandwidth),
            output_frequency_harmonic,
        )
    else:
    ...
    if output_frequency_harmonic:
        output_harmonic = np.array(real_output_harmonic) + 1j * np.array(
            imaginary_output_harmonic
        )
    else:
